/*
 ...
 Author: Pranil Maharjan,Sabin raj Bajracharya, Nischal Pandey
 */
...
public class FileInput {
    ...
    static final Matcher matchPoint = Pattern.compile("\\s*(\\d+)[^\\d]+(\\d+)\\s*").
            matcher("");

    public static void path(String in) {

       final Matcher matchPathCmd = Pattern.compile("([MmLlHhVvAaQqTtCcSsZz])|([-+]?"
               + "((\\d*\\.\\d+)"
               + "|(\\d+))([eE][-+]?\\d+)?)").matcher(in);
        ...
        try {
            ...
            directory=directory+"/potrace/instruction.txt";
